	function C3Counter(id, opt) {
			...
			this.animateDigits = function() {
				for (var dc=0; dc<8; dc++) {
					...
					if (digits[no].digit == 0) jQuery("#digit-bg"+no).css("top", -this.options.digitImageHeight+this.options.digitHeight + "px");
					if (digits[no].digit != digits[no].digitNext) {
						jQuery("#digit-bg"+no).animate( { "top" : -digits[no].digitNext*options.digitHeight+"px"}, options.digitSlideTime);
